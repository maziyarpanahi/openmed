...
- native Swift GLiNER zero-shot NER over the de-identified note
...
5. Press the main action button to run OCR, de-identification, and the GLiNER pass in sequence.
6. The app uses `OpenMed/OpenMed-PII-LiteClinical-Small-66M-v1-mlx` to mask PII and `OpenMed/gliner-multi-pii-v1-mlx` to run a second zero-shot extraction pass on the masked note.
7. Enter a Hugging Face token in the GLiNER card the first time if the private artifact is not cached yet.
8. Each artifact is cached locally after a successful download. Later runs reuse the cached copy and only fetch files again if a previous download was interrupted and left the cache incomplete.
9. To test disconnected mode, run the demo once while online so both artifacts are cached, then disable network access and run the same sample or scan flow again.
...
- native Swift GLiNER extraction over a masked note
...
- The selected OpenMed PII model is public, so no Hugging Face token is required for de-identification.
- The GLiNER artifact is private for now. The first download needs a Hugging Face token, but later runs can stay fully offline from cache.
