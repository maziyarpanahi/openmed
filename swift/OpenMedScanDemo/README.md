...
6. The app uses `OpenMed/OpenMed-PII-LiteClinical-Small-66M-v1-mlx` or `OpenMed/privacy-filter-mlx` to mask PII and `OpenMed/gliner-relex-base-v1.0-mlx` to extract clinical entities from the masked note.
7. Enter a Hugging Face token in the setup sheet the first time if the private clinical artifact is not cached yet.
...
- The selected OpenMed PII model is public, so no Hugging Face token is required for that default de-identification path.
- OpenAI Privacy Filter and GLiNER Relex artifacts are private for now. Their first download needs a Hugging Face token in internal builds, but later runs can stay fully offline from cache.
