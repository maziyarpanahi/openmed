...
- Added deterministic per-workflow rollups of agent run events with run counts,
  outcome counts, tool-call counts, durations, and unique artifact digests,
  rendered in workflow-identifier order as byte-stable JSON and Markdown that
  reconcile exactly with the overall run summary, with synthetic regression
  tests (#3041).
