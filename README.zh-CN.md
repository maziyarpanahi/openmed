...
`am`、`ar`、`cs`、`da`、`de`、`el`、`en`、`es`、`fr`、`he`、`hi`、`id`、`it`、`ja`、`ko`、`mr`、`nl`、`no`、`pt`、`ro`、`ru`、`sv`、`sw`、`ta`、`te`、`th`、`tr`、`uk`、`xh`、`zh` 和 `zu`，共计 **600+ 个 PII 检查点**。
俄语和中文路由目前使用文档中说明的多语言默认模型占位符，专用模型权重仍单独提供。
一个由用户选择并配置的印度语言 NER 系列还支持七条额外路由
（`as`、`bn`、`gu`、`kn`、`ml`、`or` 和 `pa`），
并且也可服务于印地语、马拉地语、泰米尔语和泰卢固语。请设置 `OPENMED_INDIC_NER_MODEL`；
