...
class ColdStartBudgetTest {
    ...
    fun libraryInitializationStaysWithinCommittedBudget() {
        ...
        catalog = ModelCatalog.load(context)
        val cache = ModelCache(cacheDirectory)
        downloader = ModelDownloader(cache, NetworkRejectingClient)
        val elapsedNanos = System.nanoTime() - startedAtNanos
        val elapsedMillis = (
            elapsedNanos + NANOS_PER_MILLISECOND - 1
        ...
        assertTrue(
            "The packaged model catalog must not be empty",
            catalog.entries.isNotEmpty(),
        )
        assertTrue(
            "Model cache setup must create its app-private directory",
