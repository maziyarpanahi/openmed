...
def _default_runner(
    ...
    *,
    ...
    try:
        completed = subprocess.run(  # noqa: S603 - explicit caller-supplied argv
            list(command),
            input=text,
        ...
        raise MedCATBridgeError(
            "MedCAT process timed out; no output was accepted"
        ) from exc
    except UnicodeError as exc:
        raise MedCATBridgeError(
            "MedCAT process returned output that was not valid UTF-8"
        ) from exc
    except OSError as exc:
        raise MedCATBridgeError(
            "The configured MedCAT process could not be started; install MedCAT "
            "separately and verify the command"
        ) from exc
    if completed.returncode != 0:
        raise MedCATBridgeError(
            "MedCAT process exited unsuccessfully; stderr was suppressed to "
            "prevent source-derived data from entering logs"
        )
...
def run_medcat(
    ...
    *command* must point at a script/CLI the caller has installed and
    ...
    raw_output = active_runner(
        validated_command, text=text, timeout=timeout, cwd=cwd, extra_env=env
    )
    if not isinstance(raw_output, str):
        raise MedCATBridgeError("MedCAT process must return UTF-8 text output")
...
def _record_to_span_code(
    ...
    cui = record.get("cui")
    if isinstance(cui, bool) or not isinstance(cui, (str, int)) or not str(cui):
    ...
    if score is not None:
        if isinstance(score, bool):
            raise MedCATBridgeError("MedCAT concept record has an invalid score")
        try:
            normalized_score = float(score)
        except (TypeError, ValueError) as exc:
            raise MedCATBridgeError(
                "MedCAT concept record has an invalid score"
            ) from exc
    ...
    if name is not None:
        if not isinstance(name, str):
            raise MedCATBridgeError("MedCAT concept record has an invalid name")
        payload["name"] = name
