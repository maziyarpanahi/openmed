...
def load_suite_fixtures(name: str, **kwargs: Any) -> list[Any]:
    ...
    if suite == N2C2:
        year = str(kwargs.get("year", kwargs.get("corpus_year", ""))).strip()
        if year == "2018":
            return load_n2c2_2018_relation_fixtures(kwargs.get("path"))
        if year == "2022":
            return load_n2c2_2022_relation_fixtures(kwargs.get("path"))
        raise ValueError("n2c2 relation suite requires year=2018 or year=2022")
    if suite == SHIELD:
...
def suite_metadata(name: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if suite == N2C2:
        ...
        if year in {"2018", "2022"}:
            return dua_relation_suite_metadata(f"n2c2-{year}")
        raise ValueError("n2c2 relation suite metadata requires year=2018 or year=2022")
    if suite == SHIELD:
