...
def bidi_control_wrapping_perturbation(
    *,
    ...
    contains control characters that break naive matching.
    """
...
def adversarial_perturbation_report(
    ...
    *,
    ...
    model_name, model_runner = _resolve_model_runner(model, runner)
    base_runner = model_runner or default_model_runner
    clean = _suite_leakage_metrics(fixtures, base_runner, model_name, device)

    variants: list[AdversarialPerturbationVariant] = []
    violations: list[str] = []
    for index, perturbation in enumerate(
        _resolve_adversarial_perturbations(perturbations)
    ):
        ...
        leakage = _suite_leakage_metrics(
            perturbed,
            base_runner,
            model_name,
            device,
...
def _resolve_adversarial_perturbations(
    perturbations: (
        PerturbationLike
        | Sequence[PerturbationLike]
        | Mapping[str, PerturbationLike]
    ...
    """Resolve Unicode operators without changing the base-suite defaults."""
    if perturbations is None:
        return DEFAULT_ADVERSARIAL_PERTURBATIONS
    if isinstance(perturbations, (str, Perturbation)):
        return (_coerce_adversarial_perturbation(perturbations),)
    if isinstance(perturbations, Mapping):
        return tuple(
            _coerce_adversarial_perturbation(perturbation, name=name)
            for name, perturbation in perturbations.items()
        )
    return tuple(
        _coerce_adversarial_perturbation(perturbation) for perturbation in perturbations
    )


def _coerce_adversarial_perturbation(
