...
class RxNormLinker:
        ...
        *,
        ...
        for concept in self._vocab.lookup_all(query):
            best[concept.code] = Candidate(
                SYSTEM, concept.code, concept.preferred_term, 1.0
            )
            ...
            for entry, score in self._approximate(query):
                ...
                if existing is None or score > existing.score:
                    best[entry.code] = Candidate(
                        SYSTEM, entry.code, entry.preferred_term, score
                    )
    ...
    def _approximate(self, query: str) -> list[tuple[VocabConcept, float]]:
        try:
            matches = self._vocab.fuzzy_lookup(query)
