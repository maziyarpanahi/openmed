...
BLOCK_ONLY_MODE: Literal["block_only"] = "block_only"
REDACT_TO_STAGING_MODE: Literal["redact_to_staging"] = "redact_to_staging"
DEFAULT_MODE: GuardMode = BLOCK_ONLY_MODE
...
def redact_text(text: str, findings: Iterable[ScannerFinding]) -> str:
    ...
    try:
        ...
        raise DatasetGuardError(
            "dataset findings could not be normalized safely"
        ) from None
    if not normalized:
        return text
...
def scan_dataset_files(
    ...
    *,
    ...
    The scanner receives file text only.  File content is hashed for the report;
    source paths are never passed to the scanner or copied into reports or
    exception messages.
    """
...
class DatasetUploadGuard:
    ...
    def __init__(
        ...
        *,
        ...
        self._upload = upload
        self.mode: GuardMode = _normalize_mode(mode)
