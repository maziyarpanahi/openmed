...
class TestLocaleResolution:
    ...
    def test_kannada_pack_warns_once_and_preserves_initial_name_shape(self):
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            assert resolve_locale("kn") == "kn_IN"
            anonymizer = Anonymizer(lang="kn", consistent=True, seed=689)
            native = anonymizer.surrogate("ಕೆ. ಎಸ್. ರವಿ", "PERSON")
            latin = anonymizer.surrogate("K. S. Ravi", "PERSON")
        ...
        assert len(native.split()) == 3
        assert len(latin.split()) == 3
        assert all(part.endswith(".") for part in native.split()[:2])
        assert all(part.endswith(".") for part in latin.split()[:2])
        assert any("\u0c80" <= character <= "\u0cff" for character in native)
        assert all(character.isascii() for character in latin)
        assert native != "ಕೆ. ಎಸ್. ರವಿ"
        assert latin != "K. S. Ravi"
        assert validate_kannada_aadhaar(aadhaar)
