...
class TestLocaleResolution:
    ...
    def test_punjabi_pack_warns_once_and_uses_gender_matched_names(self):
        pack = get_language_pack("pa")

        assert pack is not None
        assert pack.scripts == ("Gurmukhi",)
        assert "pa" in SUPPORTED_LANGUAGES
        assert DEFAULT_PII_MODELS["pa"] == "OpenMed/privacy-filter-multilingual"
        assert LANGUAGE_NAMES["pa"] == "Punjabi"
        assert LANGUAGE_MODEL_PREFIX["pa"] == "Punjabi-"
        assert LANGUAGE_MONTH_NAMES["pa"] == [
            "ਜਨਵਰੀ",
            "ਫ਼ਰਵਰੀ",
            "ਮਾਰਚ",
            "ਅਪ੍ਰੈਲ",
        ...
        assert LANG_TO_LOCALE["pa"] == "pa_IN"
        assert NATIONAL_ID_PROVIDERS["pa"] == ("pa_IN", "aadhaar")
        assert FAKER_BACKEND_LOCALE["pa_IN"] == "en_IN"
        assert "pa" in L._APPROXIMATE_LOCALES

        L._warned.clear()
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            assert resolve_locale("pa") == "pa_IN"
            assert resolve_locale("pa") == "pa_IN"
            anonymizer = Anonymizer(lang="pa", consistent=True, seed=691)
            masculine = anonymizer.surrogate("ਜੱਸਪ੍ਰੀਤ ਸਿੰਘ", "PERSON")
            feminine = anonymizer.surrogate("ਫ਼ਤਿਹ ਕੌਰ", "PERSON")
        ...
        assert masculine.endswith(" ਸਿੰਘ")
        assert feminine.endswith(" ਕੌਰ")
        assert masculine != "ਜੱਸਪ੍ਰੀਤ ਸਿੰਘ"
        assert feminine != "ਫ਼ਤਿਹ ਕੌਰ"
