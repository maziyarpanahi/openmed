...
from openmed.core.labels import ACCOUNT_NUMBER, API_KEY, ID_NUM, OTHER, SSN
...
def test_tiny_distill_preset_declares_directid_contract_compatibility():
    ...
    assert set(DIRECTID_TINY_HEAD_CONTRACT.critical_labels) <= set(
        config.loss.critical_labels
    )
