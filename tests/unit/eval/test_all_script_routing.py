...
def test_each_run_resolves_to_the_expected_language_path():
    ...
    for run in fixture["runs"]:
        ...
        assert decision.script == run["script"]
        assert decision.candidates == tuple(run["candidate_languages"])
        assert decision.language == run["selected_language"]
        assert decision.source == run["route_source"]
        if run["resolution"] == "selected":
            ...
            assert decision.source in {
                "stdlib:unknown-script",
                "stdlib:arabic-fallback",
            }
