...
def test_multiple_findings_bind_nearest_mixed_units_in_source_order():
    ...
    assert [
        (item["finding"], item["laterality"], item["size_value"], item["size_unit"], item["location"])
...
def test_bilateral_and_unknown_laterality_are_explicit():
    assert extract_radiology_findings("Bilateral pulmonary nodules.")[0]["laterality"] == "bilateral"
    assert extract_radiology_findings("A lesion is present.")[0]["laterality"] == "unknown"
...
def test_provenance_spans_index_into_original_text():
    ...
    assert text[spans["size"]["start"] : spans["size"]["end"]] == "6 mm"
...
def test_optional_radlex_mapping_is_caller_supplied_and_case_insensitive():
    finding = extract_radiology_findings("A nodule.", radlex_mapping={"NODULE": "RID4278"})[0]
...
def test_order_is_deterministic_and_empty_input_is_empty():
    ...
    assert [item["finding"] for item in extract_radiology_findings(text)] == ["nodule", "mass"]
