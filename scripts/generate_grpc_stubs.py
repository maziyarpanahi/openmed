...
OPENMED_PROTO_FILE = PROTO_DIR / "openmed.proto"
KSERVE_PROTO_FILE = PROTO_DIR / "kserve_v2.proto"
...
def _generate_into(output_dir: Path) -> None:
    ...
    openmed_args = [
        ...
        str(OPENMED_PROTO_FILE),
    ]
    result = protoc.main(openmed_args)
    if result != 0:
    ...
    kserve_args = [
        ...
        f"-I{grpc_tools_include}",
        f"--python_out={output_dir}",
        str(KSERVE_PROTO_FILE),
    ]
    result = protoc.main(kserve_args)
    if result != 0:
        raise SystemExit(result)
    ...
    (output_dir / "__init__.py").write_text(
        ...
        "from . import kserve_v2_pb2, openmed_pb2, openmed_pb2_grpc\n\n"
        '__all__ = ["kserve_v2_pb2", "openmed_pb2", "openmed_pb2_grpc"]\n',
        encoding="utf-8",
    )
