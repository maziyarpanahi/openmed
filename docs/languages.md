...
the built-in Assamese, Bengali, Hindi, Kannada, Marathi, Odia, Tamil, and
Telugu codes.
...
`user-supplied` (`gu`, `ml`, `pa`, `ne`, and `ur`) ship no bundled
...
- `gu`, `ml`, and `pa` read `env:OPENMED_INDIC_NER_MODEL`. They resolve
