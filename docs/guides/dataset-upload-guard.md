...
The source files are not modified. Staged files are atomically replaced and
use owner-only permissions on POSIX systems:
...
it does not contain the original or staged path. File identifiers are derived
from file content, so identical files intentionally receive the same identifier.
