...
Eight additional Indic codes (`as`, `bn`, `kn`, `ml`, `mr`, `or`, `pa`, and
`ta`) are opt-in routes through a user-configured
`OPENMED_INDIC_NER_MODEL`; Gujarati, Hindi, and Telugu can use the same adapter.
