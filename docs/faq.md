...
PII extraction and de-identification support **33 supported PII language codes**:
...
Five additional Indic codes (`as`, `gu`, `kn`, `ml`, and `pa`)
are opt-in routes through a user-configured `OPENMED_INDIC_NER_MODEL`;
Bengali, Hindi, Marathi, Tamil, and Telugu can use the same adapter.
...
[Multilingual PII](https://github.com/maziyarpanahi/openmed#multilingual-pii-33-supported-languages).
