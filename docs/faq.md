...
Seven additional Indic codes (`as`, `gu`, `kn`, `ml`, `mr`, `or`, and `pa`)
are opt-in routes through a user-configured `OPENMED_INDIC_NER_MODEL`;
Bengali, Hindi, Tamil, and Telugu can use the same adapter.
...
[Multilingual PII](https://github.com/maziyarpanahi/openmed#multilingual-pii-31-supported-languages).
