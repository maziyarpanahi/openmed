...
`id`, `it`, `ja`, `ko`, `nl`, `no`, `pt`, `ro`, `ru`, `sv`, `sw`, `ta`, `te`,
...
optional Indic routes (`as`, `bn`, `gu`, `kn`, `ml`, `mr`, `or`, and `pa`)
when `OPENMED_INDIC_NER_MODEL` or an explicit model is configured; Hindi,
Tamil, and Telugu can use that adapter too. `confidence_threshold` defaults to
`0.5`.
