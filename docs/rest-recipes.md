...
codes: `am`, `ar`, `cs`, `da`, `de`, `el`, `en`, `es`, `fr`, `gu`, `he`, `hi`,
`id`, `it`, `ja`, `ko`, `nl`, `no`, `pt`, `ro`, `ru`, `sv`, `sw`, `te`, `th`,
`tr`, `uk`, `xh`, `zh`, and `zu`. Russian and Chinese currently use
documented multilingual default-model placeholders. The API also accepts eight
optional Indic routes (`as`, `bn`, `kn`, `ml`, `mr`, `or`, `pa`, and `ta`) when
`OPENMED_INDIC_NER_MODEL` or an explicit model is configured; Gujarati, Hindi,
