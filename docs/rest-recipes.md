...
`id`, `it`, `ja`, `ko`, `nl`, `no`, `pa`, `pt`, `ro`, `ru`, `sv`, `sw`, `te`,
...
optional Indic routes (`as`, `bn`, `gu`, `kn`, `ml`, `mr`, `or`, and `ta`) when
`OPENMED_INDIC_NER_MODEL` or an explicit model is configured; Hindi, Punjabi,
