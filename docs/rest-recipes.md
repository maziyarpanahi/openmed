...
`id`, `it`, `ja`, `ko`, `nl`, `no`, `or`, `pt`, `ro`, `ru`, `sv`, `sw`, `te`,
`th`, `tr`, `uk`, `xh`, `zh`, and `zu`. Russian and Chinese currently use
documented multilingual default-model placeholders. The API also accepts eight optional
Indic routes (`as`, `bn`, `gu`, `kn`, `ml`, `mr`, `pa`, and `ta`) when
