...
| DICOM, contacts, and PDFs | DICOM header de-identification, burned-in pixel OCR redaction, vCard/iCalendar PHI redaction, deterministic redacted-PDF rendering, global text-removal verification, and masked layout-fidelity regression gates. | `openmed/multimodal/dicom.py`, `openmed/multimodal/contacts_calendar.py`, `openmed/multimodal/render_pdf.py`, `openmed/multimodal/verify_pdf.py`, [Redacted PDF Rendering And Fidelity](./multimodal/pdf-redaction-fidelity.md) |
