...
**Cause.** PII extraction and de-identification support **30 supported PII
language codes: am, ar, as, cs, da, de, el, en, es, fr, he, hi, id, it, ja,
ko, nl, no, pt, ro, ru, sv, sw, te, th, tr, uk, xh, zh, and zu**. Russian
and Chinese currently use documented multilingual default-model placeholders.
Passing anything outside that set (or a mistyped code) raises this error.
The accepted API set also includes eight optional Indic routes: `bn`, `gu`,
`kn`, `ml`, `mr`, `or`, `pa`, and `ta`. Those codes require an explicit model
or `OPENMED_INDIC_NER_MODEL`; Assamese, Hindi, and Telugu can use the adapter.
