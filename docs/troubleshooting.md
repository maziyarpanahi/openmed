...
language codes: am, ar, cs, da, de, el, en, es, fr, he, hi, id, it, ja, kn,
ko, nl, no, pt, ro, ru, sv, sw, te, th, tr, uk, xh, zh, and zu**. Russian and
...
`gu`, `ml`, `mr`, `or`, `pa`, and `ta`. Those codes require an explicit model
or `OPENMED_INDIC_NER_MODEL`; Hindi, Kannada, and Telugu can use the adapter too.
