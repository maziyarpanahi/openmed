...
ml, nl, no, pt, ro, ru, sv, sw, te, th, tr, uk, xh, zh, and zu**. Russian
...
The accepted API set also includes eight optional Indic routes: `as`, `bn`,
`gu`, `kn`, `mr`, `or`, `pa`, and `ta`. Those codes require an explicit
model or `OPENMED_INDIC_NER_MODEL`; Hindi, Malayalam, and Telugu can use the
adapter too.
