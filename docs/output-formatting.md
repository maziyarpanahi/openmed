...
- `analyze_text` attaches sentence spans (when pySBD is enabled or when using the experimental yasbd backend.) and forwards `metadata` objects so each entity can carry
